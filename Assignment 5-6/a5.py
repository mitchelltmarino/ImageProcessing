...
for key, symbolDict in symbolData.items():
    ...
    print('feature:', feature_array[currentSymbol])
    
    # add symbol to FeatureList.txt
    f = open('FeatureList.txt', 'a')
    entry = '{},'.format(IMAGE_NAME.split('/')[1].split('.')[0])
    for i in range(ZONES - 1):
        ratio = feature_array[currentSymbol][i]
        entry += '{},'.format(ratio)
    ratio = feature_array[currentSymbol][ZONES-1]
    entry += '{}\n'.format(ratio)
